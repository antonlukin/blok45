...
class Blok45_Modules_Global {
    ...
    public static function load_module() {
        ...
        add_action( 'admin_bar_menu', array( __CLASS__, 'add_adminbar_clear' ), 1000 );
        add_action( 'admin_init', array( __CLASS__, 'clear_nginx_cache' ) );
    ...
    /**
     * Create admin bar button to clear static cache
     */
    public static function add_adminbar_clear( $adminbar ) {
        if ( current_user_can( 'manage_options' ) ) {
            $adminbar->add_menu(
                array(
                    'id'    => 'clear_nginx_cache',
                    'title' => esc_html__( 'Очистить кэш Nginx', 'blok45' ),
                    'href'  => wp_nonce_url( admin_url( '?clear_nginx_cache=1' ), 'clear_nginx_cache_action' ),
                    'meta'  => array( 'title' => esc_html__( 'Очистить кэш Nginx', 'blok45' ) ),
                )
            );
        }
    }
    ...
    /**
     * Send shell signal to clear nginx cache
     */
    public static function clear_nginx_cache() {
        ...
        if ( current_user_can( 'manage_options' ) && check_admin_referer( 'clear_nginx_cache_action' ) ) {
            shell_exec( 'sudo /srv/http/options/clear-nginx-cache.sh' ); // phpcs:ignore
